...
def build_class_nav(base_dir):
    ...
    if os.path.exists(os.path.join('docs', section_index)): nav.append({'Lessons Index': section_index})

    for folder in folders:
        folder_path = os.path.join(base_dir, folder)
        items = []
        index_path = os.path.join(folder_path, 'index.md')
        if os.path.exists(index_path): items.append({'Index': index_path.replace('docs/', '')})
        ...
        if len(items) == 1: nav.append({f"Class {folder.split('_')[1]}": list(items[0].values())[0]})
        else: nav.append({f"Class {folder.split('_')[1]}": items})
    return nav

def build_file_nav(directory):
    nav = []
    index_path = os.path.join(directory, 'index.md')
    if os.path.exists(index_path): nav.append({'Index': index_path.replace('docs/', '')})
